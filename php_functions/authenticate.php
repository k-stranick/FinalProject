...
{
    $stmt = $db_conn->prepare("UPDATE users SET failed_attempts = 0, last_failed_attempt = NULL WHERE id = ?");
...
{
    $stmt = $db_conn->prepare("UPDATE users SET failed_attempts = failed_attempts + 1, last_failed_attempt = NOW() WHERE id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $db_conn->prepare("SELECT id, password_hash, failed_attempts, last_failed_attempt FROM users WHERE username = ?");
